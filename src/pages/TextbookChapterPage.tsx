...
// Lazily loaded zone components per data chapter ID
const zones: Record<string, Record<ZoneNum, React.LazyExoticComponent<() => React.ReactElement>>> = {
  ...
  '11': { 1: lazy(() => import('../zones/ch11/Zone1')), 2: lazy(() => import('../zones/ch11/Zone2')), 3: lazy(() => import('../zones/ch11/Zone3')), 4: lazy(() => import('../zones/ch11/Zone4')), 5: lazy(() => import('../zones/ch11/Zone5')) },
  '12': { 1: lazy(() => import('../zones/ch12/Zone1')), 2: lazy(() => import('../zones/ch12/Zone2')), 3: lazy(() => import('../zones/ch12/Zone3')), 4: lazy(() => import('../zones/ch12/Zone4')), 5: lazy(() => import('../zones/ch12/Zone5')) },
  '13': { 1: lazy(() => import('../zones/ch13/Zone1')), 2: lazy(() => import('../zones/ch13/Zone2')), 3: lazy(() => import('../zones/ch13/Zone3')), 4: lazy(() => import('../zones/ch13/Zone4')), 5: lazy(() => import('../zones/ch13/Zone5')) },
}

export default function TextbookChapterPage() {
  const { tbId = '1', zone = '1' } = useParams<{ tbId: string; zone: string }>()
  const zoneId = Math.max(1, Math.min(5, Number(zone))) as ZoneNum
  const tbChapter = TEXTBOOK_CHAPTERS.find(c => c.tbId === Number(tbId))
  if (!tbChapter) return <div style={{ color: 'var(--color-text-muted)', padding: '2rem' }}>Chapter not found.</div>

  const dataId = String(tbChapter.dataChapterId)
  const ZoneComponent = zones[dataId]?.[zoneId]
  if (!ZoneComponent) return <div style={{ color: 'var(--color-text-muted)', padding: '2rem' }}>Content coming soon.</div>
