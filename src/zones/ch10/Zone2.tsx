...
const DATASETS = {
  legitimate: {
    label: 'Legitimate Revenue Data (1,000 invoices)',
    description: 'Distribution closely follows Benford\'s Law — leading 1s appear ~30% of the time.',
    data: [302, 178, 124, 96, 80, 68, 58, 52, 42],
  },
  fabricated: {
    label: 'Fabricated Expense Data (1,000 entries)',
    description: 'Human-generated numbers cluster around middle digits (4-7) — a classic fraud signal.',
    data: [182, 145, 128, 148, 142, 135, 58, 40, 22],
  },
  roundNumbers: {
    label: 'Round-Number Journal Entries (1,000 entries)',
    description: 'Entries ending in 000 show unusual 1s and 5s — suggesting management override.',
    data: [285, 45, 35, 40, 148, 35, 198, 38, 176],
  },
}
...
export default function Zone2() {
  ...
  const chiSquared = useMemo(() => {
    return dataset.data.reduce((sum, observed, i) => {
      const expected = (BENFORD_EXPECTED[i] / 100) * total
      return sum + Math.pow(observed - expected, 2) / expected
    }, 0)
  }, [dataset, total])

  const isSuspicious = chiSquared > 15.51 // χ² critical value at α=0.05, df=8

  return (
    <SimulationWrapper
      title="Benford's Law Fraud Detector"
      description="Compare first-digit distributions of different datasets against Benford's Law. Fabricated financial data deviates from the expected distribution — which is how forensic accountants detect manipulation statistically."
    >
          ...
          return (
            <div key={i} style={{ textAlign: 'center' }}>
              ...
              <div style={{ height: '120px', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: '2px' }}>
                <div style={{ width: '12px', background: '#1e3a5f40', height: `${(expectedPct / 35) * 100}%`, borderRadius: '2px 2px 0 0' }} title={`Expected: ${expectedPct}%`} />
                <div style={{ width: '12px', background: barColor, height: `${(observedPct / 35) * 100}%`, borderRadius: '2px 2px 0 0' }} title={`Observed: ${observedPct.toFixed(1)}%`} />
              </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', fontSize: '0.7rem', color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)' }}>
        ...
        <span>■ <span style={{ color: '#dc2626' }}>Suspicious deviation (&gt;5%)</span></span>
      </div>
      ...
      <div style={{ padding: '0.875rem 1.125rem', background: isSuspicious ? '#dc262612' : '#1b433212', border: `1px solid ${isSuspicious ? '#dc262630' : '#1b433230'}`, borderRadius: '0.5rem', fontSize: '0.82rem' }}>
        ...
        <div style={{ color: 'var(--color-text-muted)' }}>
          ...
          {isSuspicious ? 'The distribution deviates significantly from Benford\'s Law — this warrants forensic investigation.' : 'This distribution is statistically consistent with naturally occurring financial data.'}
        </div>
      </div>
    </SimulationWrapper>
  )
