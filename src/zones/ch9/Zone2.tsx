...
export default function Zone2() {
  ...
  return (
    <SimulationWrapper
      title="Acquisition Accounting Calculator"
      description="Walk through a purchase price allocation (PPA). Adjust the deal parameters to see how goodwill is calculated — and what percentage of the purchase price becomes an intangible that must eventually be tested for impairment."
    >
      ...
      {Number(goodwillPct) > 40 && (
        <div style={{ padding: '0.75rem 1rem', background: '#dc262612', border: '1px solid #dc262630', borderRadius: '0.5rem', fontSize: '0.8rem', color: '#dc2626' }}>
          ⚠ <strong>High goodwill concentration ({goodwillPct}% of purchase price)</strong> — this deal depends heavily on synergies materializing. If the $
          {goodwill}M goodwill is impaired, it will flow directly through the income statement with no cash impact but significant equity destruction.
        </div>
      )}
    </SimulationWrapper>
  )
